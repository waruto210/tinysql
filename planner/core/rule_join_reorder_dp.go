...
func (s *joinReorderDPSolver) solve(joinGroup []LogicalPlan, eqConds []expression.Expression) (LogicalPlan, error) {
	...
	for _, node := range joinGroup {
		...
		s.curJoinGroup = append(s.curJoinGroup, &jrNode{p: node, cumCost: s.baseNodeCumCost(node)})
	}
	adjMatrix := make([][]int, len(joinGroup))
	...
	for _, cond := range eqConds {
		...
		totalEqEdges = append(totalEqEdges, joinGroupEqEdge{nodeIDs: []int{lIndex, rIndex}, edge: f})
	...
	for i := 0; i < len(joinGroup); i++ {
		...
		// 用bfs找到从i开始能join到的所有表（一个连通分量，笛卡尔积没有join条件会导致不连通）
